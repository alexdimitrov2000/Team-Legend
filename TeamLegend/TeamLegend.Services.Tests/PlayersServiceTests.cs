namespace TeamLegend.Services.Tests
{
    ...
    public class PlayersServiceTests
    {
        ...
        [InlineData("")]
        [InlineData("   ")]
        public async Task GetByNameAsync_WithInvalidNameParam_ReturnsNull(string name)
        {
            var context = new ApplicationDbContext(this.Options);

            var playersService = new PlayersService(context);

            var resultPlayer = await playersService.GetByNameAsync(name);
        ...
        [InlineData("abc", "abv")]
        [InlineData("playerId", "playerName")]
        public async Task GetByNameAsync_WithExistingPlayer_ReturnsExistingPlayer(string id, string playerName)
        {
            var context = new ApplicationDbContext(this.Options);
            ...
            var resultPlayer = await playersService.GetByNameAsync(playerName);
        ...
        public async Task GetByNameAsync_WithExistingPlayersWithSameName_ReturnsFirstOccurrenceOfExistingPlayer(string playerName)
        {
            var context = new ApplicationDbContext(this.Options);

            var playersService = new PlayersService(context);

            var firstPlayer = new Player { Name = playerName };
            var secondPlayer = new Player { Name = playerName };

            await context.Players.AddAsync(firstPlayer);
            await context.Players.AddAsync(secondPlayer);
            await context.SaveChangesAsync();

            var resultPlayer = await playersService.GetByNameAsync(playerName);

            Assert.NotNull(resultPlayer);
            Assert.Equal(firstPlayer.Id, resultPlayer.Id);
            Assert.Equal(firstPlayer.Name, resultPlayer.Name);
        }
        ...
        [InlineData("abc", "abv")]
        [InlineData("playerId", "playerName")]
        public async Task GetByNameAsync_WithInexistentPlayer_ReturnsNull(string id, string playerName)
        {
            var context = new ApplicationDbContext(this.Options);
        ...
        [InlineData("abc", "abv")]
        [InlineData("playerId", "playerName")]
        public async Task GetByNameAsync_WithExistingPlayerButInconsistentName_ReturnsNull(string id, string playerName)
        {
            var context = new ApplicationDbContext(this.Options);
            ...
            var resultPlayer = await playersService.GetByNameAsync(playerName + "null");
